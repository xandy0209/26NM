...
import { SearchIcon, PlusIcon, EditIcon, TrashIcon, XIcon, DownloadIcon, CloudUploadIcon } from './Icons';
...
const MOCK_PERSONNEL: Personnel[] = [
    { id: '1', name: '张三', phone: '13800138000', city: '呼和浩特' },
    { id: '2', name: '李四', phone: '13900139000', city: '包头' },
];
...
export const CustomerResponsePersonnelView: React.FC = () => {
    ...
    const handleEdit = (p: Personnel) => {
        setEditingId(p.id);
        setFormData({ name: p.name, phone: p.phone, city: p.city });
        setIsModalOpen(true);
    };
    ...
    const handleConfirmDelete = () => {
        if (deletingId) {
            setPersonnelList(prev => prev.filter(p => p.id !== deletingId));
            setDeletingId(null);
        }
    };
    ...
    return (
        <div className="relative flex-1 flex flex-col h-full overflow-hidden bg-transparent border border-blue-500/30 shadow-[inset_0_0_20px_rgba(0,133,208,0.1)]">
            <div className="bg-transparent p-3 border-b border-blue-500/20 flex flex-wrap items-center justify-between gap-3 shrink-0">
                ...
                <StyledButton variant="primary" icon={<PlusIcon />} onClick={handleAdd}>新增人员</StyledButton>
            </div>
                ...
                <table className="w-full text-left text-sm whitespace-nowrap border-separate border-spacing-0">
                    ...
                    <tbody className="text-white">
                        {paginatedList.length > 0 ? paginatedList.map(p => (
                            ...
                            <tr>
                                <td colSpan={4} className="p-8 text-center text-blue-300/50 border-b border-blue-500/10">暂无数据</td>
                            </tr>
                        )}
                    </tbody>
                </table>
            ...
            <div className="bg-transparent p-3 border-t border-blue-500/20 flex items-center justify-between shrink-0">
                <div className="flex items-center gap-3">
                    <input 
                        type="file" 
                        ref={fileInputRef} 
                        ...
                        className="hidden" 
                    />
                    <StyledButton variant="secondary" icon={<DownloadIcon />} onClick={handleDownloadTemplate}>导入模版</StyledButton>
                    <StyledButton variant="secondary" icon={<CloudUploadIcon />} onClick={handleImportClick}>导入</StyledButton>
                </div>
                <Pagination 
                ...
                <div className="absolute inset-0 z-[50] flex items-center justify-center bg-black/60 backdrop-blur-sm">
                    <div className="bg-[#0b1730] border border-blue-500/30 p-6 w-[400px] shadow-[0_0_30px_rgba(0,210,255,0.2)]">
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                <label className="block text-xs text-blue-300 mb-1">电话 <span className="text-red-500">*</span></label>
                                <StyledInput 
                                    className="w-full"
            ...
            {deletingId && (
                <div className="absolute inset-0 z-[50] flex items-center justify-center bg-black/60 backdrop-blur-sm">
                    <div className="bg-[#0b1730] border border-blue-500/30 p-6 w-[320px] shadow-[0_0_30px_rgba(0,210,255,0.2)]">
                        ...
                        <p className="text-blue-200 text-sm mb-6">确定要删除该客响人员吗？此操作不可恢复。</p>
